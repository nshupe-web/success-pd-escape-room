...
import { getAllMissions, getTeamByCode, getMission, updateTeamProgress } from '@/lib/firebase-utils';
...
export default function DashboardPage() {
  ...
  const loadTeamData = useCallback(async () => {
    ...
    try {
      ...
      if (!mission) {
        ...
        const finalMissionId = sortedMissions.at(-1)?.id;

        if (sortedMissions.length === 0) {
          setSetupMessage('No missions have been added yet. Open the admin panel and create Mission 1 before teams begin.');
        } else if (finalMissionId && teamData.currentMission > finalMissionId) {
          setIsVictory(true);
        } else {
          setSetupMessage(`Mission ${teamData.currentMission} was not found in Firebase. Add that mission in the admin panel or reset this team to an existing mission.`);
        }
      } else {
  ...
  if (setupMessage && team) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <Card className="w-full max-w-lg bg-gray-900/80 border-yellow-500/30">
          <CardHeader>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="rounded-lg border border-gray-800 bg-gray-950/70 p-4 text-sm text-gray-400">
              <p>
                Team loaded: <span className="text-white font-medium">{team.name}</span>
              </p>
              <p>
                Current mission: <span className="text-white font-medium">Mission {team.currentMission}</span>
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/admin" className="flex-1">
                <Button className="w-full bg-yellow-500 hover:bg-yellow-600 text-gray-950 font-semibold">
                  Open Admin Panel
                </Button>
              </Link>
              <Button
